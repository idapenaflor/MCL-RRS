<?php
	...
	{
		$getNotifs = GetAdminNotification($con,$account,$dept);
	    ...
	    for($ctr=0; $ctr<count($arrayID); $ctr++)
	    {
	    	...
	    	$getNotifs = GetRequestDetails($con,$arrayID[$ctr]);
	    	//echo count($getNotifs);
		    ...
		    $isnotify = GetNotifsss($con,$account,$arrayID[$ctr]);
		    ...
		    */
		    ...
		    switch($account)
		    {
		    	case 'Dean':
		    	{
		    		...
		    		$getTime = DeanTime($con,$arrayID[$ctr]);
		    	...
		    	case 'LMO':
		    	{
		    		$getTime = LMOTime($con,$arrayID[$ctr]);
		    	...
		    	case 'CDMO':
		    	{
		    		$getTime = CDMOTime($con,$arrayID[$ctr]);
	...
	{
		$getNotifs = GetStaffNotification($con,$account,$userid);
	    ...
	    for($ctr=0; $ctr<count($arrayID); $ctr++)
	    {
	    	...
	    	{
	    		$getTime = GetStaffApproved($con,$arrayID[$ctr]);
	    		if(mysqli_num_rows($getTime) > 0)
			    {
			        while($row = mysqli_fetch_array($getTime))
			        {
	    	...
	    	{
	    		$getTime = GetStaffRejected($con,$arrayID[$ctr]);
